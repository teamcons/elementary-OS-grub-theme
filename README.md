# elementary OS grub theme

<a href="https://elementary.io"><img alt="Static Badge" src="https://img.shields.io/badge/Designed_for_elementary_OS-%239bdb4d?style=flat&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%%2BCjwvc3ZnPgo%3D&link=https%3A%2F%2Felementary.io%2F"></a>


Grub menu theme for Elementary OS 8, Circe, and beyond!

# THE WIP WORKS, BUT THE CODE IS UGLY


<div align="center">
    <img src="https://github.com/UncleTallest/elementary-OS-grub-theme/blob/main/screenshot.jpg" /></td>
</div>

## ToDO

Also put a better screenshot or something<br>
Maybe a Menu, font size, and logo more in line with current eOS (plymouth...)<br>
A better install script.<br>
Have the entry detect OS version <br>


## Installation

1. Download ZIP. Unpack it. Open the folder

2. Right-click, "Open in Terminal" and run the command:

```bash
$ sudo install.sh
```

If you give it as argument the path to a png, it will use it as logo
WARNING: for now, only 191x191 PNGs...

3. Reboot


## Uninstallation

1. Open the folder

2. Right-click, "Open in Terminal" and run the command:

```bash
$ sudo uninstall.sh
```

3. Reboot


